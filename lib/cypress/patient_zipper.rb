...
module Cypress
  ...
  class QRDAExporter
    ...
    def initialize(measures, start_time, end_time)
      ...
      C3EXPORTER.load_measures_and_value_sets(@measures_json, @valueset_json_map[@measures.first.bundle_id])
    end
    ...
    def export(patient)
      ...
      when 'r3'
        C3EXPORTER.export_with_ffi(patient.to_json(:include => :provider), start_time, end_time, 'r3', cms_compatibility)
      when 'r3_1'
        C3EXPORTER.export_with_ffi(patient.to_json(:include => :provider), start_time, end_time, 'r3_1', cms_compatibility)
      end
    end
  end
